...
const STEPS = [
  { id: 'basic', label: 'Basic Info', icon: User, step: 1 },
  { id: 'project', label: 'Project Details', icon: FileText, step: 2 },
  { id: 'location', label: 'Location', icon: MapPin, step: 3 },
  { id: 'documents', label: 'Documents', icon: Upload, step: 4 },
  { id: 'fees', label: 'Fees', icon: Calculator, step: 5 },
  { id: 'compliance', label: 'Compliance', icon: Shield, step: 6 }
];
...
export function TabbedPermitForm({ permitId, onComplete }: TabbedPermitFormProps) {
  ...
  const validateCurrentStep = () => {
    switch (currentStep) {
      case 1: // Basic Info
        return formData.entity_id && formData.title;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const finalCompletedSteps = [1, 2, 3, 4, 5, 6];
      await saveDetails({
        ...details,
        ...formData,
        current_step: 6,
            ...
            <>
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <Label className="text-base font-medium">Entity Selection *</Label>
                  ...
                  <div className="border rounded-lg p-4">
                   <EntitySelector
                     ...
                     onEntitySelect={(entityId, entityData) => {
                       console.log('🏢 TabbedPermitForm - EntitySelector onEntitySelect:', { entityId, entityData });
                       updateFormData({ 
                         entity_id: entityId,
                         entity_type: entityData?.entity_type || null,
                         entity_name: entityData?.name || null // Save entity name
                       });
                     }}
                     onEntityCreate={() => {
                       // Optionally refresh entities or show success message
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={formData.title || permitInfo?.title || ''}
